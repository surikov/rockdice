...
'use strict'
console.log('WebAudioFont Channel v1.05 RockDice');
...
'use strict'
console.log('WebAudioFont Loader v1.30 RockDice v2.3');
...
function WebAudioFontLoader(player) {
	...
	this.insRealURL=function(key){
		...
		//console.log('replacement',replacement.includes(key),key)
		if(replacement.includes(key)){
			return './waf/ins/' + key + '.js';
		}else{
			return 'https://surikov.github.io/webaudiofontdata/sound/' + key + '.js';
		}
	}
	...
	this.drumInfo = function(n) {
		...
		return {
			...
			//url: './waf/drums/128' + key + '.js',
			url: this.drumRealURL( key ),
			pitch: p,
...
'use strict'
console.log('WebAudioFont Player v2.82++ RockDice');
...
function WebAudioFontPlayer() {
	...
	this.queueWaveTable = function (audioContext, target, preset, when, pitch, duration, volume, slides) {
		...
		volume = this.limitVolume(volume);
		var tremorRatio = 0.1;
		volume = volume + Math.random(volume * tremorRatio) - 0.5 * tremorRatio;
...
'use strict'
console.log('WebAudioFont Reverberator/Compressor v1.13 RockDice');
